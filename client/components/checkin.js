...
const CheckIn = (props) => {
  const { toggleCheckIn, toggleCheckOut } = props;
  return (
    <div>
      <span className="small-text">Dates</span>
      <div id="checkin-dates-container">
        <span className="checkin-text" onClick={toggleCheckIn}>Check-In</span>
        ...
        <span className="checkin-text" onClick={toggleCheckOut}>Check-out </span>
      </div>
      <span className="small-text">Guests</span>
      <div id="guests-container" />
      <div>
        {props.checkin ? <Calendar /> : null }
      </div>
    </div>
  );
};
